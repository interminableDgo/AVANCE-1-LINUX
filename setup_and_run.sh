...
setup_influxdb() {
    ...
    # Asegurar bucket procesado de forma robusta
    local INFLUX_TOKEN="PpCwdSIMJdtVNgnnghBtDll0Q7KKRWzOm-LrSyCAOEo5jaVix2-NP0VPNkCoM_ztd4ZzsZzuyPi5Iuk9CD0ZCg=="
    if _docker exec my-influxdb influx bucket find -n my_app_processed_data -o my-org -t "$INFLUX_TOKEN" >/dev/null 2>&1; then
        success "Bucket my_app_processed_data ya existe"
    else
        log "Creando bucket my_app_processed_data..."
        if _docker exec my-influxdb influx bucket create -n my_app_processed_data -o my-org -t "$INFLUX_TOKEN" >/dev/null 2>&1; then
            success "Bucket my_app_processed_data creado"
        else
            warning "No se pudo crear bucket (posible existencia previa). Continuando..."
        fi
    fi
